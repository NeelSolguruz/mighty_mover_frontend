...
import {
  ...
  Packer_data,
  House_Shifting_Services_with_Best_Packers_and_Movers_in_Ahmedabad,
  packers_image_desc,
  Need_an_Extra_Hand,
  Need_an_Extra_Hand_data,
  Need_hand_img_title_desc,
  Other_Services_to_Choose_From,
  Some_words_from_our_happy_customers,
      ...
      <div className="w-full flex flex-col gap-8 ">
        ...
        <div className="w-full ">
          <div className="w-full flex justify-center items-center p-4 pt-[20px] gap-20 max-[769px]:gap-10 max-[633px]:gap-4 max-[597px]:flex-col">
            {packers_image_desc.map((item) => (
              <>
                <div className="flex flex-col justify-center items-center w-[300px] gap-2 ">
                  <div>
                    <Image src={item.img} alt={item.title} className="max-[597px]:w-24 max-[507px]:h-24 "></Image>
                  </div>
        ...
        <div className="w-full mt-10">
          <div className="flex flex-col justify-center items-center gap-1">
            <div className="text-2xl font-bold">{Need_an_Extra_Hand}</div>
            <div className="text-base font-normal text-center p-2">
              {Need_an_Extra_Hand_data}
            </div>
          </div>
        </div>
        <div className="w-full mt-1 flex justify-center">
          <div className="grid grid-cols-2 grid-rows-2 justify-items-center gap-x-16 gap-y-4 w-[664px] max-[676px]:grid-cols-1 max-[676px]:gap-y-10">
            {Need_hand_img_title_desc.map((item) => (
              <>
                <div className="w-[290px] flex flex-col justify-center items-center gap-2 ">
                  <div>
                    <Image src={item.img} alt={item.title} className="max-[676px]:w-24 max-[676px]:h-24"></Image>
                  </div>
                  <div className="text-[#2D2F34] font-base text-center font-semibold">
                    {item.title}
                  </div>
                  <div className="font-base text-center">{item.desc}</div>
                </div>
              </>
            ))}
          </div>
        ...
        <div className="w-full flex justify-center mt-16 p-2">
          <div className="text-2xl font-bold text-center">
            {Some_words_from_our_happy_customers}
          </div>
        </div>
        <div className="w-full px-20 max-[1100px]:px-10 ">
          <div
          ...
          >
            {REVIEWS.map((item, index) => (
              <div key={index}>
                <div
                  className="p-4 min-h-72 rounded-xl
                  hover:scale-105 hover:shadow-md hover:shadow-gray-400 transition-all"
                >
                  <div className="flex">
                    <Image
                      src={item.img}
                      ...
                      // width={80}
                      // height={20}
                      className="rounded-full w-[40px] h-[40px] mt-4 mr-1 "
                    />
                    <div className="ml-1 mt-1 text-[#2D2F34]">
                      <p className="text-base font-semibold">{item.name}</p>
                      <p className="text-sm text-[#2D2F34]">
                        {Packers_Movers_title},{item.rating}
                        <span className="text-yellow-500">&#9733;</span>
                      </p>
                      <p className="text-sm">{item.loc}</p>
                    </div>
                  </div>
                  <div className="mt-2 text-xl text-[#2D2F34]">
                    <p>{item.desc}</p>
                  </div>
                </div>
              </div>
            ...
            {ENTERPRISE_STRING.ENTERPRISE_FAQ.map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  <svg
                    className={`fill-blue-500 shrink-0 ml-8 transition-transform duration-200 ${
